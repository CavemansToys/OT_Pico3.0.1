...
#include "encoder.h"  // For encoder_event_queue and ButtonEncoderEvent_t
#include "lvgl_port.h"  // For ui_init()
...
// LVGL display buffer size - reduced for RAM constraints
// Single buffer of 10 lines: 480 * 10 * 2 bytes = 9600 bytes
...
// encoder_event_queue is declared in encoder.h

// LVGL display buffer (single buffer to save RAM)
...
void tft35_module_init(void) {
    ...
    lv_display_set_buffers(display, lvgl_buf1, NULL, sizeof(lvgl_buf1),
