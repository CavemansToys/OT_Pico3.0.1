...
static void update_timer_cb(lv_timer_t *timer) {
    ...
    // Update drop count
    snprintf(buf, sizeof(buf), "Drop %d / %d", session->drops_completed, session->max_drops_allowed);
    ...
    if (ai_tuning_is_active() && session->drops_completed > 0) {
        uint8_t idx = (session->drop_write_idx > 0) ? session->drop_write_idx - 1 : AI_TUNING_DROP_BUF_SIZE - 1;
        ai_drop_telemetry_t *last_drop = &session->drops[idx];

        snprintf(buf, sizeof(buf), "Last: Kp=%.2f Kd=%.2f",
                 last_drop->coarse_kp_used, last_drop->coarse_kd_used);
        lv_label_set_text(overthrow_label, buf);
