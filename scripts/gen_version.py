...
GIT_VERSION_PATTERN_REGEX = r'v(?P<major>\d+)\.(?P<minor>\d+)(\.(?P<micro>\d+))?-(?P<patch>\d+)-g(?P<short_hash>[a-f0-9]+)(?P<dirty>-dirty)?'
...
def main(output_path, build_type):
    ...
    if match:
        ...
        if micro:
            version_string = f"{major}.{minor}.{micro}{dirty_string}"
        else:
